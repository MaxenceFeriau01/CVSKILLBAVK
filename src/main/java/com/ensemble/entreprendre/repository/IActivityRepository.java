...
public interface IActivityRepository extends JpaRepository<Activity, Long>, PagingAndSortingRepository<Activity, Long>,
	...
	@Query("SELECT new com.ensemble.entreprendre.projection.CustomActivity(a.id as id, a.name as name, count(c) as companyCount, count(u) as userCount, count(s) as companySearchCount) "
			...
			+ "WHERE UPPER(a.name) like %:#{#name}% GROUP BY a.id ORDER BY COUNT(a.id) DESC")
